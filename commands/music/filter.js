module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (!args[0]) return message.channel.send(`❌ **|** Veuillez spécifier un filtre valide pour activer ou désactiver, ${message.author}.\n${actualFilter ? `Filtre actuellement actif ${actualFilter} (${client.config.app.px}filtre ${actualFilter} pour le désactiver).\n` : ''}`);
